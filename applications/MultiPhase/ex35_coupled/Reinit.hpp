...
class Reinit {
    public:
        ...
        void computeMarkersAdvection(std::vector<std::vector<double>>& global_markers) {
            ...
            const auto converged_flag = projectPointsOnInterface(global_markers);
            ...
            for (std::size_t a = 0; a < _d; a++)
                for (std::size_t i = 0; i < converged_flag.size(); i++) {
                    ...
                    if(conv_flag == 1)
                        converged_global_markers[a].push_back(global_markers[a][i]);
                }

            global_markers.swap(converged_global_markers);
