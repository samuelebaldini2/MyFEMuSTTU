...
int run_ex35(const SimConfig& cfg) {
  ...
  std::vector<unsigned> reinit_cnt(delta_depth, 0);
  ...
  for (unsigned levelN = levelNstart; levelN < levelNstart + delta_depth; levelN++) {
    ...
    double eps = (dim == 2)
      ? 1. / pow(2, std::max(levelN - 7u, 1u))
      : 1. / pow(2, std::max(levelN - 4u, 1u));
    ...
    const unsigned psiId0_init = field0[topLevel].id("Psi");
    auto& Psi0 = field0[topLevel].getById(psiId0_init);
    for (std::size_t k = 0; k < Psi0.size(); ++k) {
      const std::vector<double> x = field0[topLevel].dofCoordById(psiId0_init, k);
      Psi0[k] = psi2D(x);
    }
    ...
    Reinit reinit_init(elProj, field0, psiId0_init, psi2D._m);
    reinit_init.computeMarkersAdvection(markers);
    ...
    for (unsigned k = 1; k <= nSteps; ++k) {
      ...
      {
        const unsigned psiId0_curr = field0[topLevel].id("Psi");
        Reinit reinit(elProj, field0, psiId0_curr, psi2D._m);
      ...
      if (k % print_step == 0) std::cout << "\x1b[1A" << "\x1b[2K";
    ...
    // BEGIN MERGE INITIAL AND FINAL SOLUTIONS
    {
      ...
      const auto& Xtop = mesh2[topLevel].X();
      out.clear();
      pl.locateAll(out, Xtop);
